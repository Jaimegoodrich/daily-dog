...
export function ClientList() {
  ...
  const filtered = clients?.filter((c) =>
    c.main_name.toLowerCase().includes(search.toLowerCase())
  )

  return (
    <div>
      <div className="mb-6 flex items-center justify-between gap-4">
        <h1 className="font-display text-2xl font-extrabold text-ocean-900">👤 Clients</h1>
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered?.length === 0 && (
          <p className="text-center text-ocean-700/50">No clients found.</p>
        )}
      </div>
    </div>
  )
